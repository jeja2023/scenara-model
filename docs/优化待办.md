...
- 完整发布与升级说明见 [发布说明_0.8.0.md](发布说明_0.8.0.md)。
...
- 完整发布与升级说明见 [发布说明_0.7.0.md](发布说明_0.7.0.md)。
...
**问题**：`更新日志.md` 已发布 `[0.6.0]`，而 `pyproject.toml` / `__init__.py` / 前端 `package.json` 都是 `0.5.0`，`README.md` 停在 `0.4.1` 且仍描述「可选 Bearer Token 鉴权」（实际已是强制登录认证）。
...
```markdown
当前版本：`0.6.0`。完整变更见 [更新日志.md](更新日志.md)。
```
...
`docs/架构设计.md` 的 API 清单在 `GET /health` 后插入 `POST /api/auth/login`、`POST /api/auth/logout`、`GET /api/auth/me`。

`docs/生产交付验收.md` 环境变量表在 `SCENARA_MODEL_AUTH_TOKEN` 后补：
...
`docs/运行维护手册.md:70` 与 `docs/生产交付验收.md:97` 那句已经过时的描述改为：
...
def main() -> int:
    ...
    if latest is None or latest.group(1) != expected:
        problems.append(f"更新日志.md 最新条目 = {latest.group(1) if latest else '缺失'}")
...
**问题 2**：`docker-compose.yml` 的 `postgres` / `minio` profile 实际不可用——镜像不含 `[postgres]` / `[s3]` extras，主服务也没有 `depends_on` 和 DSN 覆盖，但 `docs/生产交付验收.md:77` 把它写成「可选生产形态」。
...
`docs/生产交付验收.md:77` 的命令同步改为上面的三段式写法。
