...
Model 的生产认证由 `scenara` Core 统一提供。生产设置 `SCENARA_MODEL_AUTH_MODE=core` 后，服务使用独立的服务凭据和上下文 HMAC 校验 Core 透传的租户、项目、主体、权限范围及产品授权；本地用户名密码和 Session 仅保留在 `local` 迁移期模式。Model 获取训练输入时，通过 `SCENARA_MODEL_DATA_PLATFORM_URL`、独立 Data 服务凭据和 Data 上下文签名，从 `scenara-data` 读取已发布 Dataset Version 的引用与不可变清单，不接受本地路径冒充生产数据版本。
...
- 标注规范和质检报告。
- 训练、评估、导出入口。
- YOLO 检测、ReID、分类、分割本地基线适配器；生产环境可通过 `ultralytics_yolo`、`fastreid`、`torchreid`、`torchvision_classifier`、`segmentation_framework` adapter 配置外部 argv 命令接入真实框架。`fastreid` 还要求固定源码提交、环境锁、数据物化目录、CMC+mAP 固定测试协议和许可证溯源。
- ONNX 文件和标准模型包。
...
- `POST /api/v1/deployment-feedback`（Core 签名 Webhook；公开路径但必须通过 HMAC、时间窗和事件信封校验）
- `GET /api/v1/deployment-feedback`（已认证的事件审计查询）
...
- `fastreid`：训练阶段必须先执行 `training.data_prepare_command` 并生成 `training.prepared_dataset_root`；评估必须使用 `fastreid-cmc-map-v1`、`dataset.test_manifest` 和实测 `map`、`rank1`、`rank5`、`rank10`。框架 Git 提交及环境锁摘要写入 checkpoint 溯源。
...
Core `deployment-feedback` 以至少一次投递方式发送。消费端在解析 JSON 前校验 `Scenara-Event-Id`、`Scenara-Timestamp`、`Scenara-Signature` 和原始 body 的 HMAC-SHA256；相同 event ID 只处理一次，迟到事件不能覆盖较新的 `(tenant_id, project_id, capability)` 状态，声明的前序状态不匹配时仅记录审计。
