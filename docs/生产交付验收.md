...
生产模式还必须设置 `SCENARA_MODEL_DEPLOYMENT_PROFILE=production`、`SCENARA_MODEL_AUTH_MODE=core`、`SCENARA_MODEL_SERVICE_TOKEN`、`SCENARA_MODEL_CONTEXT_SIGNING_KEY`、`SCENARA_MODEL_DATA_PLATFORM_URL`、`SCENARA_MODEL_DATA_PLATFORM_SERVICE_TOKEN` 和 `SCENARA_MODEL_DATA_PLATFORM_CONTEXT_SIGNING_KEY`。这些值支持对应的 `_FILE` secret 挂载；生产不得启用 `SCENARA_MODEL_SERVE_FRONTEND`。
...
| `SCENARA_MODEL_DEPLOYMENT_FEEDBACK_SECRET` | 空 | Core `deployment-feedback` Webhook 的 HMAC 共享密钥；未配置时端点失败关闭 |
| `SCENARA_MODEL_DEPLOYMENT_FEEDBACK_MAX_AGE_SECONDS` | `300` | 签名 Webhook 可接受的时间窗（秒） |
...
python scripts/hash_artifact.py 训练标注与模型研发配套方案.md
...
FastReID 生产训练使用独立 Linux GPU 容器：
...
容器预检通过后，准备含 `person_id`、`camera_id` 和 query/gallery 角色的不可变 ReID manifests，再运行 `configs/experiments/reid_fastreid_external.yml`。完整步骤见 [FastReID训练运行手册](FastReID训练运行手册.md)。

PostgreSQL 和 S3/MinIO 目标环境必须用独立凭据完成读写资格探针，并在恢复库完成备份恢复演练：

```powershell
$env:SCENARA_MODEL_S3_ENDPOINT_URL="https://<s3-endpoint>"
$env:SCENARA_MODEL_S3_ACCESS_KEY_ID="<access-key>"
$env:SCENARA_MODEL_S3_SECRET_ACCESS_KEY="<secret-key>"
python scripts/qualify_target_environment.py `
  --metadata-db "postgresql://<user>:<password>@<host>:5432/<recovery-db>" `
  --storage-backend minio `
  --storage-uri "minio://<bucket>/models" `
  --keep-object-evidence
```

`backup_postgres.py` 拒绝覆盖既有 archive；`restore_postgres.py` 要求 SHA-256 和 `--confirm-recovery-target`，只能指向经批准的恢复目标。
...
- FastReID 生产训练还必须固定 Linux 容器、框架提交和环境锁摘要，使用 `training.data_prepare_command` 物化本地 JPEG 训练/查询/gallery 布局，回传 checkpoint、ONNX、`mAP`、Rank-1、Rank-5 和 Rank-10；Windows 冒烟只能验证开发机功能。
- Core 部署反馈仅接受签名 `model.deployment.changed` Webhook。必须配置 `SCENARA_MODEL_DEPLOYMENT_FEEDBACK_SECRET`；平台拒绝无效/过期签名、event ID 载荷冲突和错误状态重放。
- PostgreSQL/S3/MinIO 目标交付前必须执行读写探针及 SHA-256 备份恢复演练，保留报告和探针对象或等效审计证据。
...
- Python 包、运行时 `__version__`、前端包和 lockfile 已统一为 `1.0.0-dev.6` / `1.0.0.dev6`，并由 `scripts/check_versions.py` 自动校验。
- 接入 scenara Core 委托身份上下文统一认证（HMAC-SHA256 签名与时间戳校验），支持短期服务凭据与租户/项目权限校验。
- 实现 `scenara_model.data_platform.DataPlatformClient`，支持直接从已发布的 `scenara-data` 数据集版本拉取引用及不可变清单。
- 确立 `production` 与 `development` 部署 profile 隔离，生产环境强制 Core 认证、PostgreSQL 元数据存储与 S3/MinIO 对象存储。
- 生产环境 Docker Compose（`deploy/compose.production.yml`）加固落地：启用 `read_only` 根只读文件系统、`cap_drop: ALL`、`no-new-privileges`，并通过 Docker Secrets 挂载敏感凭据。
- 编写生产环境 Compose 静态安全门禁校验脚本 `scripts/production_gate.py` 并接入 GitHub CI 工作流。
- 环境变量配置全面对齐：`.env` 与 `.env.example` 保持完全一致，涵盖 Core 认证、Data 数据服务对接与生产凭据声明。
- 流水线启动模板明确区分真实生产训练与自检冒烟基线，执行日志与归档产物标签全面中文化与语义高亮。
- 完整说明见 `docs/发布说明_1.0.0-dev.6.md`。

## 1.0.0-dev.5 发布门禁

- Python 包、运行时 `__version__`、前端包和 lockfile 已统一为 `1.0.0-dev.5` / `1.0.0.dev5`，并由 `scripts/check_versions.py` 自动校验。
- 前端交互与视觉规范全面升级：落地多区域 Tab 选项卡（`TabBar`）、全边框 42px 行高数据表格、26px 统一操作按钮、单行截断与 Portal 自定义气泡（`Tooltip`）。
- 全局数据表格首列统一设立 `序号` 列，并与分页 `(currentPage - 1) * pageSize + index + 1` 连续累加联动。
- 前端文案 100% 汉化覆盖，时间格式统一为东八区无 T 时间（`YYYY-MM-DD HH:mm:ss`）。
- 选择文件按钮样式微质感优化，去除系统原生灰框且严格保持原有小巧尺寸。
- `.env` 逐行对齐 `.env.example`，补全部署反馈密钥与过期参数，相关测试通过。
- 前端 `npm run build`（TypeScript 严格模式 + Vite 8 打包）0 报错，后端测试正常通过。
- 完整说明见 `docs/发布说明_1.0.0-dev.5.md`。

## 1.0.0-dev.4 发布门禁

- Python 包、运行时 `__version__`、前端包和 lockfile 已统一为 `1.0.0-dev.4` / `1.0.0.dev4`，并由 `scripts/check_versions.py` 自动校验。
- FastReID 生产 adapter、Linux GPU 容器、manifest 物化、实测 CMC+mAP、环境/许可证溯源门禁均已实现；Windows CUDA FastReID 冒烟仅作功能验证。
- Core `deployment-feedback` 签名 Webhook 已接入，覆盖签名、时间窗、event ID 幂等、状态有序回放与审计持久化。
- PostgreSQL 18.4 custom-format 备份/恢复，以及 PostgreSQL + MinIO 写读探针已在隔离环境演练通过；详细结果见 [资格验证报告](资格验证报告_2026-09-04.md)。
- `.venv\Scripts\python.exe -m pytest` 为 `116 passed`；Ruff、Pyright（0 errors）、`pip check`、版本门禁和两套 Compose 配置解析通过。本次离线验收重跑仅被现有 detection smoke experiment 锁阻塞，其余步骤通过；释放该任务锁后应重跑完整验收。
- Linux FastReID 容器实测、真实业务数据/固定评估集、权属原件和目标环境容量仍是外部资格项，当前成熟度保持 `seed`。
- 完整说明见 `docs/发布说明_1.0.0-dev.4.md`。
...
- 完整说明见 `docs/发布说明_1.0.0-dev.3.md`。
...
- 完整说明见 `docs/发布说明_1.0.0-dev.2.md`。
...
- 完整说明见 `docs/发布说明_1.0.0-dev.1.md`。
## 0.3.0 发布门禁
