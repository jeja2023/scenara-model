...
## 1.0.0-dev.5：管理台体验升级与工业化前端规范

- 管理台引入工业化数据表格规范：所有表格首列统一设立 `序号` 列并联动分页递增，单元格行高统一 42px，单元格内按钮统一 26px，单行不换行截断配合自定义 Portal `Tooltip` 悬浮气泡，时间统一为东八区无 T 格式。
- 多区域页面（概览、模型包、流水线、实验、数据标注）全面引入 `TabBar` 选项卡切换，消除长页面纵向堆叠。
- 部署反馈相关密钥已同步至 `.env` 与 `.env.example`，日常启动直接执行 `python start.py` 自动构建前端并托管体验。

## 1.0.0-dev.4：FastReID、部署反馈与目标环境

- FastReID 的正式训练环境为独立 Ubuntu/Linux GPU 容器；构建、预检、manifest 物化、训练、评估和 ONNX 导出步骤见 [FastReID训练运行手册](FastReID训练运行手册.md)。`training.data_prepare_command` 会把不可变 ReID manifests 物化为 FastReID 需要的本地 JPEG 训练/query/gallery 布局。
- Windows `.venv-fastreid-smoke` 只用于验证 CUDA、FastReID 模型构建与前向推理。它不可用于生产训练、实测指标或模型发布。
- 接收 Core `model.deployment.changed` 前，设置 `SCENARA_MODEL_DEPLOYMENT_FEEDBACK_SECRET`。Webhook 不使用迁移期本地登录鉴权：它在读取 JSON 前校验 HMAC、时间戳、事件 ID 和信封一致性；重复 event ID 幂等成功，迟到或前序状态不一致的事件不会覆盖当前状态。
- PostgreSQL 与 S3/MinIO 目标环境先运行 `scripts/qualify_target_environment.py`，再通过 `scripts/backup_postgres.py` 和 `scripts/restore_postgres.py` 在批准的恢复库演练。恢复必须提供 archive SHA-256 和 `--confirm-recovery-target`。
- 本机验证结果及仍需外部提供的资格材料见 [资格验证报告](资格验证报告_2026-09-04.md)。
