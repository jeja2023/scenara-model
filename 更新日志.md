...
- **东八区无 T 时间格式化工具**：新增 `formatBeijingTime` 工具函数（`frontend/src/utils/date.ts`），统一将 UTC / ISO 时间解析为 `Asia/Shanghai` 东八区时间，格式统一输出为 `YYYY-MM-DD HH:mm:ss`，彻底移除字符 `T`、毫秒及冗余时区偏移。
